...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  ...
  | Mul(x, y') -> (Ans(Mul (find x Type.Int regenv, y')), regenv)
  | Div(x, y') -> (Ans(Div (find x Type.Int regenv, y')), regenv)
  | Ld(x) -> (Ans(Ld(find x Type.Int regenv)), regenv)
  | St(x, y) -> (Ans(St(find x Type.Int regenv, find y Type.Int regenv)), regenv)
  ...
  | LdF(x) -> (Ans(LdF(find x Type.Int regenv)), regenv)
  | StF(x, y) -> (Ans(StF(find x Type.Float regenv, find y Type.Int regenv)), regenv)
