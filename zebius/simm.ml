...
and g' env = function (* 各命令の即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Arith (AMul, x, V(y)) when M.mem x env && M.mem y env -> Set (M.find x env * M.find y env)
  | Arith (ADiv, x, V(y)) when M.mem x env && M.mem y env -> Set (M.find x env / M.find y env)
  | Arith (op, x, V(y)) when M.mem y env -> Arith (op, x, C (M.find y env))
  | Arith (op, x, V(y)) when M.mem x env -> Arith (op, y, C (M.find x env))
  | Arith (AMul, x, C y) when M.mem x env -> Set (M.find x env * y) (* ad-hoc optimization for array indexing *)
  | Arith (ADiv, x, C y) when M.mem x env -> Set (M.find x env / y) (* ad-hoc optimization *)
