...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  ...
  | Arith (op, x, y') -> (Ans(Arith(op, find x Type.Int regenv, find' y' regenv)), regenv)
  ...
  | IfF0(x, e1, e2) as exp -> g'_if dest cont regenv exp (fun e1' e2' -> IfF0(find x Type.Float regenv, e1', e2')) e1 e2
...
let g_vardef { vname = x; vbody = expr } =
  let e', _regenv' = g (Id.gentmp Type.Unit, Type.Unit) (Ans(Nop)) M.empty expr in
  { vname = x; vbody = e' }

let f (Prog (vardefs, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
  Format.eprintf "register allocation: may take some time (up to a few minutes, depending on the size of functions)@.";
  let vardefs' = List.map g_vardef vardefs in
  let fundefs' = List.map h fundefs in
  let e', regenv' = g (Id.gentmp Type.Unit, Type.Unit) (Ans(Nop)) M.empty e in
  Prog (vardefs', fundefs', e')
