...
let rec g env = function (* 定数畳み込みルーチン本体 (caml2html: constfold_g) *)
  ...
  | Add (x, y) when memi x env && findi x env = 0 -> Var y (* 0 + y -> y *)
  | Add (x, y) when memi y env && findi y env = 0 -> Var x (* x + 0 -> x *)
  | Sub(x, y) when memi x env && memi y env -> Int(findi x env - findi y env)
  | Sub (x, y) when memi x env && findi x env = 0 -> Neg y (* 0 - y -> -y *)
  | Sub (x, y) when memi y env && findi y env = 0 -> Var x (* x - 0 -> x *)
  | FNeg(x) when memf x env -> Float(-.(findf x env))
  | FAdd(x, y) when memf x env && memf y env -> Float(findf x env +. findf y env)
  | FAdd (x, y) when memf x env && findf x env = 0.0 -> Var y (* 0.0 + y -> y *)
  | FAdd (x, y) when memf y env && findf y env = 0.0 -> Var x (* x + 0.0 -> x *)
  | FSub(x, y) when memf x env && memf y env -> Float(findf x env -. findf y env)
  | FSub (x, y) when memf x env && findf x env = 0.0 -> FNeg y (* 0.0 - y -> -y *)
  | FSub (x, y) when memf y env && findf y env = 0.0 -> Var x (* x - 0.0 -> x *)
